...
class GildedRoseTest(unittest.TestCase):
    ...
    def test_backstage_passes_quality_increases_by_two_with_six_days_left(self):
        items = [Item("Backstage passes to a TAFKAL80ETC concert", 6, 15)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(17, items[0].quality)

    def test_backstage_passes_quality_increases_by_three_with_five_days_left(self):
        items = [Item("Backstage passes to a TAFKAL80ETC concert", 5, 15)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(18, items[0].quality)

    def test_backstage_passes_quality_increases_by_three_with_one_day_left(self):
        items = [Item("Backstage passes to a TAFKAL80ETC concert", 1, 15)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(18, items[0].quality)

    def test_backstage_passes_quality_should_be_zero_after_event(self):
        items = [Item("Backstage passes to a TAFKAL80ETC concert", 0, 15)]
        gilded_rose = GildedRose(items)
        gilded_rose.update_quality()
        self.assertEqual(0, items[0].quality)

    def test_backstage_passes_quality_is_not_greater_than_fifty(self):
